...
function render(){const grid=document.querySelector('#productGrid'),info=document.querySelector('#resultInfo');const a=filtered();grid.innerHTML=a.length?a.map(p=>`<article class="product" data-product="${p.id}"><div class="product-art">${p.badge?`<span class="product-badge">${esc(p.badge)}</span>`:''}<span>${p.icon}</span><button class="view" data-view="${p.id}" aria-label="Lihat ${esc(p.name)}">⌕</button></div><span class="tag">${label(p.cat)}</span><h3>${esc(p.name)}</h3><p class="desc">${esc(p.desc)} · ${esc(p.unit)}</p><div class="product-bottom"><strong class="price">${rupiah(p.price)}</strong><button class="add" data-add="${p.id}" aria-label="Tambah ${esc(p.name)}">+</button></div></article>`).join(''):`<div class="empty">🔎<strong>Produk tidak ditemukan</strong><small>Coba kata kunci atau kategori lain.</small></div>`;info.textContent=`${a.length} produk · ${state.category==='all'?'semua kategori':label(state.category)}`;grid.querySelectorAll('[data-add]').forEach(b=>b.onclick=e=>{e.stopPropagation();add(Number(b.dataset.add))});grid.querySelectorAll('[data-view]').forEach(b=>b.onclick=e=>{e.stopPropagation();openDetail(Number(b.dataset.view))});grid.querySelectorAll('.product').forEach(c=>c.onclick=()=>openDetail(Number(c.dataset.product)))}
...
function checkout(){
 ...
 const phone=cleanPhone(document.querySelector('#customerPhone').value);
 const target=cleanPhone(document.querySelector('#targetInput').value);
 ...
 const hasDigital=state.cart.some(x=>DIGITAL.has(PRODUCTS.find(p=>p.id===x.id)?.cat));
 const hasPhysical=state.cart.some(x=>!DIGITAL.has(PRODUCTS.find(p=>p.id===x.id)?.cat));
 if(name.length<2||name.length>60){alert('Isi nama lengkap terlebih dahulu.');document.querySelector('#customerName').focus();return}
 if(phone.length<8||phone.length>16){alert('Isi nomor WhatsApp 8–16 digit.');document.querySelector('#customerPhone').focus();return}
 if(hasDigital&&(target.length<8||target.length>16)){alert('Untuk produk digital, isi nomor HP / ID pelanggan 8–16 digit.');document.querySelector('#targetInput').focus();return}
 ...
 const lines=state.cart.map(x=>{const p=PRODUCTS.find(p=>p.id===x.id);return `• ${p.name} (${p.unit}) x${x.qty} = ${rupiah(p.price*x.qty)}`});
 const total=state.cart.reduce((s,x)=>s+PRODUCTS.find(p=>p.id===x.id).price*x.qty,0);
 const msg=['Halo 27 Mart 👋','Saya ingin membuat pesanan:','',`Nama: ${name}`,`WhatsApp: ${phone}`,...lines,'',`Total: ${rupiah(total)}`,hasDigital?`Nomor/ID tujuan: ${target}`:null,hasPhysical?`Alamat: ${address}`:null,note?`Catatan: ${note}`:null,`Metode pembayaran: ${payment}`,'','Mohon konfirmasi pesanan dan ketersediaannya. Terima kasih.'].filter(Boolean).join('\n');
 window.open(`https://wa.me/62816951127?text=${encodeURIComponent(msg)}`,'_blank');
}
document.addEventListener('DOMContentLoaded',()=>{document.querySelectorAll('.category').forEach(b=>b.onclick=()=>setCategory(b.dataset.category));document.querySelectorAll('[data-category-link]').forEach(a=>a.onclick=e=>{e.preventDefault();setCategory(a.dataset.categoryLink)});document.querySelector('#searchForm').onsubmit=e=>{e.preventDefault();state.search=document.querySelector('#searchInput').value.toLowerCase().trim();render();document.querySelector('#produk').scrollIntoView({behavior:'smooth'})};document.querySelector('#searchInput').oninput=e=>{state.search=e.target.value.toLowerCase().trim();render()};document.querySelector('#sort').onchange=e=>{state.sort=e.target.value;render()};document.querySelector('#openCart').onclick=openCart;document.querySelector('#ctaCart').onclick=openCart;document.querySelector('#closeCart').onclick=closeCart;document.querySelector('#overlay').onclick=closeCart;document.querySelector('#checkout').onclick=checkout;document.querySelector('#clearCart').onclick=clearCart;document.querySelector('#closeModal').onclick=closeModal;document.querySelector('#productModal').onclick=e=>{if(e.target.id==='productModal')closeModal()};document.querySelector('#shopNow').onclick=()=>setCategory('all');document.querySelector('#digitalNow').onclick=()=>setCategory('pulsa');document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeCart();closeModal()}});render();renderCartCount()});
